...
use Users\Notification\NotificationManager;
...
/*    public function activationEmail()
    {   
        $userModel = $this->Auth->config('authenticate.Form.userModel');
        $userField = $this->Auth->config('authenticate.Form.fields');
//        $this->log($this->Auth->config('authenticate.Form'));
        ...
        if (!empty($this->request->data['username'])) {
            $user = $this->{$userModel}
                ->find()
                ->where([
                    $userField['username'] => $this->request->data['username'],
                    'active' => 0
                ])
                ->limit(1)
                ->first();
